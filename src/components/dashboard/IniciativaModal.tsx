...
const COLS: { label: string; render: (i: Iniciativa) => string | null }[] = [
  { label: 'Chave',        render: i => i.key },
  { label: 'Iniciativa',   render: i => i.nome },
  { label: 'Status',       render: i => i.status.name },
  { label: 'Sponsor',      render: i => i.sponsor },
  { label: 'Time',         render: i => i.timeResponsavel },
  { label: 'Domínios',     render: i => i.dominios.join(', ') || null },
  { label: 'Segmentos',    render: i => i.segmentos.join(', ') || null },
  { label: 'Benefício',    render: i => (i.beneficioQuantitativoTotal ? formatBRL(i.beneficioQuantitativoTotal) : null) },
  { label: 'Epics',        render: i => (i.epics.length ? String(i.epics.length) : '0') },
]

export default function IniciativaModal({ title, iniciativas, onClose }: Props) {
  return (
    <div
    ...
    >
      <div
        ...
        style={{ width: '92vw', maxWidth: 1100, maxHeight: '85vh' }}
          ...
          <table className="w-full text-xs border-collapse">
            ...
            <tbody>
              ...
              {iniciativas.map((init, i) => (
                ...
                >
                  {COLS.map(c => {
                    const val = c.render(init)
                    return (
                      <td
                        key={c.label}
                        className="border-b border-gray-100 text-gray-700 align-top"
                        style={{ padding: '7px 10px', maxWidth: c.label === 'Iniciativa' ? 260 : 180 }}
                      >
                        {c.label === 'Chave' ? (
