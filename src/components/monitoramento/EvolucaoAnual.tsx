...
export default function EvolucaoAnual({ epics }: EvolucaoAnualProps) {
  ...
  // ── Timeline contínua: todos os meses em sequência ──
  // Formato: [ { label: "ago/25", concluidos: 3, acumulado: 3 }, ... ]
  // Sempre considera apenas os últimos 12 meses
  const timeline = useMemo(() => {
    const hoje = new Date()
    const anoAtual = hoje.getFullYear()
    const mesAtual = hoje.getMonth() // 0-indexed

    // Gera 12 meses: do mês atual para trás
    const labels: { label: string; ano: number; mesIdx: number }[] = []
    for (let i = 11; i >= 0; i--) {
    ...
    return rows
  }, [epics, anosDisponiveis])

  // ── Agregados anuais ──
  const { totalCriados, totalConcluidos, beneficioTotal } = useMemo(() => {
    let criados = 0
    let concluidos = 0
    let benef = 0
    for (const e of epics) {
    ...
    return { totalCriados: criados, totalConcluidos: concluidos, beneficioTotal: benef }
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="bg-white rounded-lg border border-gray-200 shadow-sm p-5 text-center">
          <p className="text-3xl font-bold text-gray-800">{totalCriados}</p>
          <p className="text-xs text-gray-500 mt-1 uppercase tracking-wider">
            Experimentos criados em {anoSelecionado}
          </p>
        </div>
        <div className="bg-white rounded-lg border border-gray-200 shadow-sm p-5 text-center">
          <p className="text-3xl font-bold text-green-700">{totalConcluidos}</p>
          <p className="text-xs text-gray-500 mt-1 uppercase tracking-wider">
            Concluídos em {anoSelecionado}
          </p>
        </div>
        <div className="bg-white rounded-lg border border-gray-200 shadow-sm p-5 text-center">
          <p className="text-2xl font-bold text-purple-700">{formatCurrency(beneficioTotal)}</p>
          <p className="text-xs text-gray-500 mt-1 uppercase tracking-wider">
      ...
      <div className="bg-white rounded-lg border border-gray-200 shadow-sm">
        ...
        <div className="grid grid-cols-2 gap-0">
          <div className="p-5 text-center border-r border-gray-100">
            <p className="text-3xl font-bold text-gray-800">{ultimos12Meses.qtd}</p>
            <p className="text-xs text-gray-500 mt-1 uppercase tracking-wider">
              Experimentos criados
            </p>
          </div>
          <div className="p-5 text-center">
            <p className="text-2xl font-bold text-purple-700">{formatCurrency(ultimos12Meses.benef)}</p>
            <p className="text-xs text-gray-500 mt-1 uppercase tracking-wider">
              Benefício Potencial
            </p>
          </div>
      ...
      {/* ── Gráfico de timeline contínua ── */}
      <div className="bg-white rounded-lg border border-gray-200 overflow-hidden shadow-sm">
        <div className="px-4 py-2 border-b border-gray-100">
          <p className="text-xs font-semibold text-gray-500 uppercase tracking-widest">
            Acumulado de Experimentos Concluídos
          </p>
        </div>
        <div className="p-4">
      ...
      {/* ── Gráfico de barras: concluídos por mês (não-acumulado) ── */}
      <div className="bg-white rounded-lg border border-gray-200 overflow-hidden shadow-sm">
        <div className="px-4 py-2 border-b border-gray-100">
          <p className="text-xs font-semibold text-gray-500 uppercase tracking-widest">
            Experimentos Concluídos por Mês
          </p>
        </div>
        <div className="p-4">
